...
TEST(my_b_tree_test, check_contains) {
    ...
    ASSERT_FALSE(test_tree_node->find(1).has_value());
    ASSERT_FALSE(test_tree_node->find(-1).has_value());
    ASSERT_FALSE(test_tree_node->find(0).has_value());
    ASSERT_FALSE(test_tree_node->find(INT_MAX).has_value());
    ASSERT_FALSE(test_tree_node->find(INT_MIN).has_value());
}
